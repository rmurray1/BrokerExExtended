...
uses
  ...
  {VA}
  XWBut1, MFunStr, VWHash, VERGENCECONTEXTORLib_TLB, XWBSSOi, //XWBHash
  {Vcl}
...
public
  ...
  function    CreateContext(lHash:string; strContext: string): boolean; virtual;
...
uses
  {VA}
  LoginfrmH, RpcbErrH, SelDivH, RpcSLoginH, fRPCBErrMsg, WsockcH,
  CCOW_const, fPlinkpw, fSSHUsername, frmSignonMessageH;
...
------------------------------------------------------------------}
function TRPCBroker.CreateContext(lHash:string; strContext: string): boolean;
var
    ...
    begin                 {connect}
      ...
      try
        ...
        begin
          ...
          CreateContext(FHash, '');      //Closes XUS SIGNON context.
        end //if FSocket = 0
        else
        begin                     //p13
  ...
  begin
    ...
    begin
      ...
      if CurrentContext <> '' then         //p65 reset context if RPC errors out (context gets cleared in VistA)
        CreateContext(FHash, CurrentContext);
    end
    else
...
begin
  ...
  try
    ...
    end; //if not ConnectingBroker.KernelLogIn (silent login)
    //SSOi start
    ...
    if (not blnSignedOn) and ConnectingBroker.KernelLogIn and (ConnectingBroker.SecurityPhrase = '') then
    begin
      ...
      if (ConnectingBroker.Login.Mode = lmSSOi) and (not ConnectingBroker.KernelLogIn) then
      begin
        if ConnectingBroker.FLogin <> nil then
          blnSignedOn := SilentLogin(ConnectingBroker);   //SilentLogin in RpcSLogin unit
        ...
        begin
          ...
          if not SelDivH.ChooseDiv('',ConnectingBroker) then
          begin
            blnSignedOn := False;
    ...
    if (not blnsignedon) and ConnectingBroker.KernelLogIn then
    begin   //p13
    ...
    end; //if ConnectingBroker.KernelLogIn (A/V code failover)
  finally
    //reset the Broker
    with ConnectingBroker do
    begin
